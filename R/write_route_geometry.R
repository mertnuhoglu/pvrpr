...
write_route_geometry = function() {
	...
	writeLines(twcc$curl_cmd,glue::glue( "{PEYMAN_PROJECT_DIR}/pvrp/out/curl_cmd.sh"))
...
write_trips_with_route_geometry = function(trips_w_curl_cmd) {
	...
	sf::st_write(twrg, glue::glue("{PEYMAN_PROJECT_DIR}/pvrp/out/trips_with_route_geometry.csv"), delete_dsn = T)
}

#' @export
trips_with_curl_cmd = function() {
	c0 = readr::read_tsv(glue::glue("{PEYMAN_PROJECT_DIR}/pvrp/out/trips_with_costs.tsv")) 
		#dplyr::select(from_point_id, to_point_id, from_lng, from_lat, to_lng, to_lat)
	osrm_server = Sys.getenv("OSRM_SERVER")
	dir.create(glue::glue("{PEYMAN_PROJECT_DIR}/pvrp/out/route_json"))
	trips_w_curl_cmd = c0 %>%
		...
		dplyr::mutate(
			...
			, curl_cmd = glue::glue("curl '{route_url}' > {PEYMAN_PROJECT_DIR}/pvrp/out/route_json/{file_name}")
		)
}

#' @export
dl_route_geometry_json = function() {
	system(glue::glue("bash {PEYMAN_PROJECT_DIR}/pvrp/out/curl_cmd.sh"), intern=T)
}

#' @export
trips_with_route_geometry = function(trips_w_curl_cmd) {
	geometry = lapply(trips_w_curl_cmd$file_name, function(f) {
		j0 = rjson::fromJSON(file=glue::glue("{PEYMAN_PROJECT_DIR}/pvrp/out/route_json/{f}"))
